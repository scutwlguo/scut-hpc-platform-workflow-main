...
```text
scut-hpc-platform-workflow-main/
├── README.md
└── 华南理工大学科学计算平台演示图片/
```

> 如仓库后续新增文件或子目录，请同步更新本节目录结构。
...
> 
> **使用建议**：
> - 如果你是**初次使用**该文档尝试配置使用学校计算资源，请从头到尾认真阅读所有内容！！！！否则容易出错以及高额开销
> - 如发现文档中有错误或不当之处，欢迎指出
...
> **🚨 费用风险提醒（请务必阅读）**
> 
> 学校按**作业申请的 GPU 数量**计费（常用 A800，单卡 80G 显存，约 3.8 元/卡/小时）。
> 计费核心是“**作业数 × 每个作业申请的 GPU 张数 × 运行时长**”，**不是**“是否落在同一个计算节点”。
> 
> 例如：同时提交 4 个作业，且每个作业都 `--gres=gpu:1`，即按 4 张卡计费。
> 单日费用约为：`3.8 × 4 × 24 = 364.8 元`。
> 
> **补充：如果任务用不上 GPU，建议优先使用 CPU 分区。**
> 通用 CPU 价格约为 `0.04 元/核时`：
> - 1 核 24 小时：`0.04 × 24 = 0.96 元/天`
> - 8 核 24 小时：`0.04 × 8 × 24 = 7.68 元/天`
> 对比 GPU 成本更低，适合数据预处理、传统机器学习、CPU 可完成的推理/训练任务。
> 
> **实测示例**：以深度强化学习这类更依赖 CPU 的任务为例，在平台仅用 CPU 运行时，速度可达到个人电脑（如 5060Ti）约 2 倍。
> 这类任务通常对显存要求不高（例如显存占用长期小于 5G），优先使用 CPU 往往更划算。
> 若申请 8 核 CPU（按平台配置约对应 64G 内存），多数场景下性能和稳定性都优于本地电脑。
> 
> **为避免不必要开销，请优先采用以下策略：**
> 1. 提交前先确认当前作业数（`squeue -u $USER`）；
> 2. 若单卡显存和算力足够，优先把多个 Python 脚本放到**同一个作业脚本**里并行运行；
> 3. 在同一个作业脚本中使用后台运行 `&` + `wait`，减少重复申请 GPU；
> 4. 提交前先估算费用：`费用 = 3.8 × GPU张数 × 小时`。
> **👉 具体操作请查看第 8.3.1～8.3.4 节（先判断，再选脚本）。**
...
- [1. 平台访问与登录](#1-平台访问与登录)
   - [1.1 Web 访问](#11-web-访问)
   - [1.2 SSH 访问](#12-ssh-访问)
      - [集群1（hpckapok1）](#集群1hpckapok1)
      - [集群2（hpckapok2）](#集群2hpckapok2)
   - [1.3 桌面访问（VNC）](#13-桌面访问vnc)
      - [方式一：Web VNC](#方式一web-vnc)
      - [方式二：客户端工具](#方式二客户端工具)
- [2. 本地 IDE 远程连接配置](#2-本地-ide-远程连接配置)
   - [2.1 VSCode 远程连接](#21-vscode-远程连接)
      - [配置步骤](#配置步骤)
   - [2.2 PyCharm 远程连接](#22-pycharm-远程连接)
      - [2.2.1 配置 SFTP 连接](#221-配置-sftp-连接)
      - [2.2.2 常见问题：集群1首次 SSH 测试连接失败](#222-常见问题集群1首次-ssh-测试连接失败)
      - [2.2.3 浏览远程文件](#223-浏览远程文件)
      - [2.2.4 上传文件到远程服务器](#224-上传文件到远程服务器)
- [3. Python 环境配置](#3-python-环境配置)
   - [3.1 适用场景](#31-适用场景)
   - [3.2 两种 Anaconda 环境配置方法对比](#32-两种-anaconda-环境配置方法对比)
- [4. 方法一：独立安装 Anaconda](#4-方法一独立安装-anaconda)
   - [4.1 获取安装包](#41-获取安装包)
      - [方式 A：浏览器下载](#方式-a浏览器下载)
      - [方式 B：命令行下载](#方式-b命令行下载)
   - [4.2 安装 Anaconda](#42-安装-anaconda)
   - [4.3 创建虚拟环境](#43-创建虚拟环境)
- [5. 方法二：使用平台 Anaconda 模块（推荐）](#5-方法二使用平台-anaconda-模块推荐)
   - [5.1 完整配置流程](#51-完整配置流程)
   - [5.2 命令说明](#52-命令说明)
   - [5.3 日常使用流程](#53-日常使用流程)
- [6. GPU 版 PyTorch 安装与配置](#6-gpu-版-pytorch-安装与配置)
   - [6.1 查看 GPU 节点 CUDA 版本](#61-查看-gpu-节点-cuda-版本)
      - [6.1.1 srun 命令说明](#611-srun-命令说明)
      - [6.1.2 申请 GPU 交互式节点](#612-申请-gpu-交互式节点)
      - [6.1.3 查看 GPU 与 CUDA 信息](#613-查看-gpu-与-cuda-信息)
      - [6.1.4 退出 GPU 节点](#614-退出-gpu-节点)
   - [6.2 安装 GPU 版 PyTorch](#62-安装-gpu-版-pytorch)
   - [6.3 验证 PyTorch GPU 支持](#63-验证-pytorch-gpu-支持)
      - [6.3.1 再次申请 GPU 节点](#631-再次申请-gpu-节点)
      - [6.3.2 激活环境并验证](#632-激活环境并验证)
- [7. 常见问题与解决方案](#7-常见问题与解决方案)
   - [7.1 libtorch_cpu.so: undefined symbol: iJIT_NotifyEvent](#71-libtorch_cpuso-undefined-symbol-ijit_notifyevent)
- [8. 代码运行与 Slurm 作业提交](#8-代码运行与-slurm-作业提交)
   - [8.1 基本概念](#81-基本概念)
   - [8.2 交互式运行（调试用）](#82-交互式运行调试用)
      - [8.2.1 申请 GPU 节点](#821-申请-gpu-节点)
      - [8.2.2 执行代码](#822-执行代码)
      - [8.2.3 交互式运行时，查看 GPU 使用情况](#823-交互式运行时查看-gpu-使用情况)
   - [8.3 批处理作业方式（推荐）](#83-批处理作业方式推荐)
      - [8.3.1 先判断：选择 GPU 还是 CPU（强烈建议先看）](#831-先判断选择-gpu-还是-cpu强烈建议先看)
      - [8.3.2 GPU 作业脚本（单任务）](#832-gpu-作业脚本单任务)
      - [8.3.3 GPU 作业脚本（单作业内并行多任务）](#833-gpu-作业脚本单作业内并行多任务)
      - [8.3.4 CPU 作业脚本（仅 CPU，不申请 GPU）](#834-cpu-作业脚本仅-cpu不申请-gpu)
      - [8.3.5 提交作业与日志查看（在登录节点）](#835-提交作业与日志查看在登录节点)
      - [8.3.6 常用 Slurm 作业管理命令](#836-常用-slurm-作业管理命令)
- [9. 推荐的日常操作流程](#9-推荐的日常操作流程)
   - [9.1 普通开发环境](#91-普通开发环境)
   - [9.2 使用 GPU 时](#92-使用-gpu-时)
...
3. 在上图中第2步右边的`...`填写SSH配置集群信息（看了一下集群1的A800GPU节点数量比集群1的多，空闲的节点多点）：
- 集群1：
...
- 集群2：
   ...
   > - 建议同一个账号选择一个集群作为主要远程开发服务器，**长期使用同一个集群**可避免重复配置和环境混乱；也便于统一查看作业状态，避免多人重复提交作业造成额外费用。
   > - 如需在两个集群都使用，需分别配置并维护各自环境。
   > - 当前没有限于作者水平，暂时没有发现一键同步环境的方式，如有新解决方案欢迎补充。
   > - **注意：集群1可能出现测试连接不上的问题，详细解决方案查看2.2.2节**。
...
#### 2.2.2 常见问题：集群1首次 SSH 测试连接失败

首次在 PyCharm 中测试 SSH 连接集群1时，可能出现“测试连接失败/主机证书校验失败（KeyExchange certificate check failed）”等提示。

可先在**任一本地终端**手动建立一次 SSH 连接：
...
```bash
ssh 202420114127@202.38.252.204
```

首次连接时按提示确认主机指纹（输入 `yes`），成功登录一次后，再回到 PyCharm 测试连接通常即可通过。
...
2. **方式二**
   ...
   2.2. **根据作业ID进入对应计算节点并查看GPU使用情况**：

   ```bash
   srun --jobid=你的任务ID --pty bash
   nvidia-smi
   ```
...
#### 8.3.1 先判断：选择 GPU 还是 CPU（强烈建议先看）

如果任务**必须用 CUDA**（如大模型训练、显存密集型深度学习），选 GPU；
如果任务**不依赖 CUDA**（如数据清洗、特征工程、传统机器学习、轻量推理），优先选 CPU 分区，可显著降低费用。

> **费用参考（按你当前说明）**：
> - GPU：约 `3.8 元/卡/小时，91.2/天`
> - 通用 CPU：约 `0.04 元/核时`
> - 1 核 24 小时约 `0.96 元/天`，8 核约 `7.68 元/天`
...
##SBATCH --nodelist=g03n03           # 可选：仅用于定向调试，和计费无关
...
#### 8.3.3 GPU 作业脚本（单作业内并行多任务）

**提交前费用提醒：**

> [!WARNING]
> 费用按“作业申请 GPU 数量”计费：只要一个作业申请了 `--gres=gpu:1`，该作业就按 1 张卡计费（约 3.8 元/小时）。
> 同时提交 4 个此类作业，即按 4 张卡计费，单日约 `3.8 × 4 × 24 = 364.8 元`。

如果要同时跑多个任务，且 1 张卡足够，建议写成**一个作业脚本**，在脚本内部并行启动多个 Python 进程：

```bash
#!/bin/bash                                  

#SBATCH -J DWT_Train_multi                  # 作业名称（squeue中显示）
#SBATCH -o DWT_Train_multi.log              # Slurm标准输出日志
#SBATCH -e DWT_Train_multi.log              # Slurm错误日志（合并输出）
#SBATCH --nodes=1                           # 使用1个节点
#SBATCH --ntasks-per-node=1                 # 每个节点1个任务
#SBATCH --gres=gpu:1                        # 申请1张GPU（只收费1卡）
#SBATCH --cpus-per-task=8                   # 1张卡配8个CPU核（CPU:GPU=8:1，超出可能按更多GPU计费）
#SBATCH --partition=gpuA800                 # 指定GPU分区（A800）
#SBATCH --time=04:00:00                     # 最大运行时间4小时

module load anaconda/3-2024.02.01           # 加载Anaconda环境模块
source ~/.bashrc                            # 初始化conda环境变量
conda activate gwl                          # 激活虚拟环境gwl

cd /share/home/202420114127/gwl/code        # 进入代码目录

echo "Running on node: $SLURMD_NODENAME"    # 输出当前运行节点
echo "Start time: $(date)"                  # 输出开始时间
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"  # 查看分配的GPU编号

nvidia-smi                                  # 查看GPU状态（显存、利用率等）

export PYTHONUNBUFFERED=1                   # 关闭Python缓冲（日志实时输出）

python -u train_DWTinformer.py > DWT_run1.log 2>&1 &   # 启动第1个进程，输出到run1.log（后台运行）
python -u train_DWTinformer.py > DWT_run2.log 2>&1 &   # 启动第2个进程（后台运行）
python -u train_DWTinformer.py > DWT_run3.log 2>&1 &   # 启动第3个进程（后台运行）
python -u train_DWTinformer.py > DWT_run4.log 2>&1 &   # 启动第4个进程（后台运行）

wait                                         # 等待所有后台进程结束（必须）

echo "All tasks finished at: $(date)"        # 输出任务结束时间
```

#### 8.3.4 CPU 作业脚本（仅 CPU，不申请 GPU）

```bash
#!/bin/bash                                  

#SBATCH -J DWT_Train_CPU                     # 作业名称（squeue中显示）
#SBATCH -o DWT_Train_CPU.log                 # Slurm标准输出日志
#SBATCH -e DWT_Train_CPU.log                 # Slurm错误日志（与上面合并）
#SBATCH --nodes=1                            # 使用1个计算节点
#SBATCH --ntasks-per-node=1                  # 每个节点1个主任务（进程）
#SBATCH --cpus-per-task=8                   # 分配8个CPU核心（可按需求调整，每个核心8G内存）
#SBATCH --partition=cpuXeon6458              # 使用CPU分区（通用CPU）
#SBATCH --time=04:00:00                      # 最大运行时间4小时

module load anaconda/3-2024.02.01            # 加载Anaconda模块
source ~/.bashrc                             # 初始化conda环境
conda activate gwl                           # 激活虚拟环境
...
export CUDA_VISIBLE_DEVICES=""               # 禁用GPU（确保只用CPU）

export PYTHONUNBUFFERED=1                    # 关闭Python缓冲（日志实时输出）

python -u train_DWTinformer.py > DWT_Train_CPU.log 2>&1   # 运行脚本并输出日志（stdout+stderr）
```

#### 8.3.5 提交作业与日志查看（在登录节点）

**提交命令示例：**
首先切换到sh脚本对应的文件目录：`cd /share/home/202420114127/gwl/code `

- GPU 单任务脚本：`sbatch train_job.sh`
- GPU 多任务脚本：`sbatch train_job_multi.sh`
- CPU 脚本：`sbatch train_job_cpu.sh`
...
- GPU 单任务：`tail -f DWT_Train.log`
- GPU 多任务：`tail -f DWT_run1.log`（其余同理 `DWT_run2.log` 等）
- CPU 脚本：`tail -f DWT_Train_CPU.log`
