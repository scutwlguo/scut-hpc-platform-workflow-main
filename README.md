...
> **🚨 费用风险提醒（请务必阅读）**
> 
> 学校按**申请到的 GPU 资源数量**计费（常用 A800，单卡 80G 显存）。
> 若同一账号并行提交多个作业，且每个作业都单独申请 1 张卡，可能出现“**显存用不满但费用成倍增加**”的情况。
> 
> 例如：同时运行 4 个脚本并各申请 1 张 A800，即按 4 张卡计费；但很多非大模型任务单脚本显存可能仅占用 20G 左右，资源利用率较低。而同时4张卡差不多一个月一直运行要1w费用！
> 
> **为避免不必要开销，请严格执行以下流程（每个月学校都有每个账号使用的具体费用清单，如果不按照这个流程的话，可能费用会比较高，你懂得~！）：**
> 1. 先查看当前作业列表（使用终端命令：squeue -u $USER）；
> 2. 确认现有作业所在计算节点；
> 3. 通过终端命令检查该节点的显存占用与 GPU 利用率（详细方法看下图）；
> 4. 若资源仍有余量，优先将后续脚本指定到该节点运行。
![计算节点查看](华南理工大学科学计算平台演示图片/1.10.png)
![计算节点查看](华南理工大学科学计算平台演示图片/1.11.png)
> **👉 具体操作请查看第 8.3.2 节。**
...
- 集群2：
   ...
   > - 建议同一个账号选择一个集群作为主要远程开发服务器，**长期使用同一个集群**可避免重复配置和环境混乱。**此外，同一个账号使用同一个集群才能多人同时查看该账号的目前在该集群的作业使用情况，要不然同一个账号两个人在两个集群，就无法查看彼此作业运行的计算节点，也就无法在同一个计算节点上运行多个代码脚本，因为不同集群的计算节点是不互通的，这样会导致两个人同时使用多个计算节点，导致大额开销！！！！因此，选择集群时一定要咨询账号之前的使用者用的哪个集群**
...
## 注意："#SBATCH --nodelist=g03n03" 下面这条指令在已有作业在运行时需要通过"squeue -u $USER"在终端查看作业队列中使用的是哪个计算节点（GPU），然后把对应的计算节点名称替换下面的指定节点名称！！！！
#SBATCH --nodelist=g03n03           # 指定节点
...
> - **如果作业列表没有正在运行的作业或者已有作业所在计算节点的显存已经快占满（80G）**
>  -将作业脚本中的"#SBATCH --nodelist=g03n03           # 指定节点"命令注释，即如下脚本所示：
```bash
#!/bin/bash
#SBATCH -J DWT_Train                # 作业名称
#SBATCH -o DWT_Train.log            # 标准输出文件
...
> - **如果作业列表有正在运行的作业且已有作业所在计算节点的显存利用率很低（《80G）**
>  -将作业脚本中的"#SBATCH --nodelist=g03n03           # 指定节点"命令使用，即如下脚本所示：
